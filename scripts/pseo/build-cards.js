...
const CAPOUT = path.join(__dirname, '..', '..', 'docs', 'social')
const NUM = ['1️⃣', '2️⃣', '3️⃣', '4️⃣', '5️⃣']
function tagsFor(p) {
  const base = ['#오사카여행', '#오사카자유여행', '#일본여행', '#간사이여행', '#오사카맛집', '#일본여행코스', '#오사카가볼만한곳', '#GLUX', '#오사카가이드', '#일본자유여행']
  const kwTag = '#' + String(p.kw).replace(/[^가-힣A-Za-z0-9]/g, '')
  return [kwTag, ...base].join(' ')
}
function captions(p, items) {
  ...
  items.forEach((it, i) => {
    yt += `${i + 1}. ${it.name} (${it.area || '오사카'}) — 구글 ${it.rating}점\n   ${it.desc}\n`
  })
  yt += `\n예약·통역·전용차량은 GLUX가 도와드립니다. 문의: 프로필/설명란 링크\n${tagsFor(p)}\n`

  fs.mkdirSync(CAPOUT, { recursive: true })
